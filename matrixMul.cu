...
__global__ void matrixMul(int **a, int **b, int **c, int rc) {
...
//	col_2 = 2;
//	r_c = 3;
	int a1[4][3] = {7, 1, 3, 1, 9, 2, 5, 4, 2, 6, 9, 3};
	int b1[3][2] = {1, 5, 5, 7, 3, 4};
	int c1[4][2];
	int **d_a, **d_b, **d_c;
	...
	for (int i = 0; i < 3; i++) {
		...
		cudaMemcpy(d_a[i], a1, 12 * sizeof(int), cudaMemcpyHostToDevice);
		}
	...
	matrixMul<<<1, 8>>>(d_a,d_b,d_c,3);

	cudaMemcpy(&c1, d_c, sizeof(int), cudaMemcpyDeviceToHost);
	for(int i=0;i<8;i++)
		printf("%d ",c1[i]);
	cudaDeviceReset();
}
